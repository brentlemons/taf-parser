    ...
    constructor(tafString) {
        ...
        if (parsed) {
            ...
            if (parsed[5] != undefined && parsed[5].trim().length > 0) {
                this.forecasts = this.parseForecast(parsed[5].trim(), null, this.valid.from, this.valid.to);
            }
    ...
    parseForecast(data, currentList, tafStart, endTime) {
        ...
        if (parsed) {
            ...
            if (next != undefined && next.trim().length > 0) {
                currentList = this.parseForecast(next.trim(), currentList, tafStart, forecast.valid.from);
            }
